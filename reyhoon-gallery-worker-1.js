...
export default {
  async fetch(request, env, ctx) {
    ...
    // ---- داشبورد مدیریت وب (admin.html) — لاگین با رمز عبور جدا از تلگرام ----
...
// ============================================================
//  کاربران ثبت‌نامی (پروفایل + علاقه‌مندی‌ها + تعداد سفارش‌ها، برای ادمین)
// ============================================================
...
async function editUsersListMessage(chatId, messageId, env, page) {
  ...
  if (users.length === 0) {
    ...
    text = "👤 کاربران ثبت‌نامی (صفحه " + (page + 1) + " از " + totalPages + "، مجموع " + users.length + " نفر):";
    pageUsers.forEach((u) => {
      keyboard.push([{ text: (u.name || "بدون نام") + " — " + u.phone, callback_data: "viewuser:" + u.phone }]);
    });
  }

  const navRow = [];
  if (page > 0) navRow.push({ text: "« قبلی", callback_data: "users:" + (page - 1) });
  if (start + PAGE_SIZE < users.length) navRow.push({ text: "بعدی »", callback_data: "users:" + (page + 1) });
...
async function buildUserDetailView(phone, env) {
  const raw = await env.SHOP_DB.get("user:" + phone);
  if (!raw) {
    return { text: "این کاربر پیدا نشد.", keyboard: [[{ text: "« بازگشت به لیست", callback_data: "users:0" }, { text: "🏠 منو", callback_data: "menu" }]] };
  }
  const user = JSON.parse(raw);

  const favIds = await getUserFavorites(phone, env);
  const items = await getItems(env);
  const favNames = favIds.map((id) => {
    const it = items.find((i) => String(i.id) === String(id));
    return it ? it.name : "(محصول حذف‌شده #" + id + ")";
  });
...
async function handleCallbackQuery(cq, env) {
  ...
  if (data.startsWith("users:")) {
    const page = parseInt(data.split(":")[1]) || 0;
    await editUsersListMessage(chatId, messageId, env, page);
    return;
  }

  if (data.startsWith("viewuser:")) {
    const phone = data.slice("viewuser:".length);
    const { text, keyboard } = await buildUserDetailView(phone, env);
    await editMessage(chatId, messageId, text, env, keyboard);
    return;
  }
...
function dashboardKeyboard(role) {
  ...
  if (level >= 3) {
    rows.push([{ text: "👤 کاربران", callback_data: "users:0" }]);
  }
...
// ============================================================
//  API داشبورد مدیریت وب (admin.html)
//  لاگین با رمز عبور (env.ADMIN_PASSWORD) — جدا از سیستم همکاران تلگرام.
//  توکن حاصل از لاگین در KV با پیشوند admin_session: نگه‌داری می‌شه (۷ روز اعتبار).
//  این بخش مستقل از منطق ربات تلگرامه ولی از همون توابع/دیتای KV استفاده می‌کنه،
//  پس هر تغییری که از داشبورد وب انجام بشه (مثلاً افزودن محصول)، بلافاصله تو ربات
//  و تو سایت هم دیده می‌شه (چون همه یه KV مشترک دارن).
// ============================================================
const ADMIN_SESSION_TTL_SECONDS = 60 * 60 * 24 * 7; // ۷ روز

async function createAdminSession(env) {
  const token = generateRandomToken();
  await env.SHOP_DB.put("admin_session:" + token, "1", { expirationTtl: ADMIN_SESSION_TTL_SECONDS });
  return token;
}
...
async function isValidAdminSession(request, env) {
  ...
  const raw = await env.SHOP_DB.get("admin_session:" + token);
  return !!raw;
}
...
async function listAllOrdersAdmin(env) {
  const list = await env.SHOP_DB.list({ prefix: "order:" });
  const orders = [];
  for (const key of list.keys) {
    const raw = await env.SHOP_DB.get(key.name);
...
async function handleAdminApi(request, env, url) {
  const path = url.pathname.slice("/api/admin".length); // مثلاً "/login" یا "/products"

  // ---- لاگین: بدون نیاز به توکن ----
  if (path === "/login" && request.method === "POST") {
    if (!env.ADMIN_PASSWORD) {
      return adminJson({ error: "ADMIN_PASSWORD تنظیم نشده. اول این متغیر رو تو تنظیمات Worker (Settings → Variables and Secrets) اضافه کن." }, 500);
    }
    const body = await request.json().catch(() => ({}));
    const password = String(body.password || "");
    if (password !== String(env.ADMIN_PASSWORD)) {
      return adminJson({ error: "رمز عبور اشتباهه." }, 401);
    }
    const token = await createAdminSession(env);
    return adminJson({ token });
  }

  // ---- بقیه‌ی مسیرها همه نیاز به توکن معتبر دارن ----
  const authed = await isValidAdminSession(request, env);
  if (!authed) {
    return adminJson({ error: "وارد نشدی یا نشست منقضی شده." }, 401);
  }

  // ---------------- آمار داشبورد ----------------
  if (path === "/stats" && request.method === "GET") {
    const orders = await listAllOrdersAdmin(env);
    const users = await listRegisteredUsers(env);
    const items = await getItems(env);
    const tickets = await listAllTickets(env);
    const coAdmins = await getCoAdmins(env);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayLabel = new Date(dayStart).toLocaleDateString("fa-IR", { month: "2-digit", day: "2-digit" });
      const dayTotal = orders
        .filter((o) => o.status === "completed" && o.createdAt >= dayStart - dayMs && o.createdAt < dayStart)
        .reduce((s, o) => s + (o.total || 0), 0);
      salesByDay.push({ label: dayLabel, total: dayTotal });
    }
    ...
    return adminJson({
      counts: {
        ...
        outOfStock: items.filter((it) => typeof it.stock === "number" && it.stock <= 0).length,
  ...
  if (path === "/products" && request.method === "GET") {
    return adminJson({ items: await getItems(env) });
  }
  if (path === "/products" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    if (!body.name || !body.category || !body.karat || !body.weight) {
      return adminJson({ error: "نام، دسته، عیار و وزن الزامیه." }, 400);
    }
    const items = await getItems(env);
    const settings = await getSettings(env);
    const nextId = await getNextId(env);
    const karatVal = parseInt(body.karat);
    const defaultFee = karatVal === 24 ? settings.fee24 : settings.fee18;
    ...
    const item = {
      ...
      stock: body.stock != null ? parseInt(body.stock, 10) || 0 : 0,
      makingFee: body.makingFee != null && body.makingFee !== "" ? parseFloat(body.makingFee) : defaultFee,
  ...
  if (path === "/products/update" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const id = parseInt(body.id);
    const items = await getItems(env);
    const item = items.find((it) => it.id === id);
    if (!item) return adminJson({ error: "محصول پیدا نشد." }, 404);
    if (body.name != null) item.name = String(body.name);
    if (body.category != null) item.category = String(body.category);
    if (body.model != null) item.model = body.model ? String(body.model) : null;
    if (body.karat != null) item.karat = parseInt(body.karat);
    if (body.weight != null) item.weight = parseFloat(body.weight);
    if (body.stock != null) item.stock = parseInt(body.stock, 10) || 0;
    if (body.makingFee != null && body.makingFee !== "") item.makingFee = parseFloat(body.makingFee);
    if (body.badge != null) item.badge = body.badge ? String(body.badge) : null;
  ...
  if (path === "/products/delete" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const id = parseInt(body.id);
    ...
    await saveItems(items, env);
    return adminJson({ ok: true, deleted: before !== items.length });
  }

  // ---------------- سفارش‌ها ----------------
  if (path === "/orders" && request.method === "GET") {
    let orders = await listAllOrdersAdmin(env);
    const status = url.searchParams.get("status");
    if (status && status !== "all") orders = orders.filter((o) => o.status === status);
    return adminJson({ orders });
  }
  if (path === "/orders/status" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const ticketNumber = parseInt(body.ticketNumber);
    const newStatus = String(body.status || "");
    const valid = ["pending", "in_progress", "completed", "rejected"];
    if (!valid.includes(newStatus)) return adminJson({ error: "وضعیت نامعتبره." }, 400);
    const raw = await env.SHOP_DB.get("order:" + ticketNumber);
    if (!raw) return adminJson({ error: "سفارش پیدا نشد." }, 404);
    const order = JSON.parse(raw);
    if (newStatus === "rejected" && order.status !== "rejected") {
      try { await increaseStockForOrder(order.items, env); } catch (err) { /* بی‌خیال خطای برگردوندن موجودی */ }
    }
    order.status = newStatus;
    await env.SHOP_DB.put("order:" + ticketNumber, JSON.stringify(order));
    return adminJson({ ok: true, order });
  }
  if (path === "/orders/tracking" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const ticketNumber = parseInt(body.ticketNumber);
    const raw = await env.SHOP_DB.get("order:" + ticketNumber);
    if (!raw) return adminJson({ error: "سفارش پیدا نشد." }, 404);
    const order = JSON.parse(raw);
    order.trackingCode = String(body.trackingCode || "").trim() || null;
    order.trackingSetAt = Date.now();
    await env.SHOP_DB.put("order:" + ticketNumber, JSON.stringify(order));
    return adminJson({ ok: true, order });
  }
  if (path === "/orders/delete" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    await deleteOrder(parseInt(body.ticketNumber), env);
    return adminJson({ ok: true });
  }

  // ---------------- کاربران ----------------
  if (path === "/users" && request.method === "GET") {
    return adminJson({ users: await listRegisteredUsers(env) });
  }
  ...
  // ---------------- کدهای تخفیف ----------------
  if (path === "/discounts" && request.method === "GET") {
    const codes = await getDiscountCodes(env);
    const discounts = [];
    for (const code of codes) {
      const d = await getDiscount(code, env);
    ...
    return adminJson({ discounts });
  }
  if (path === "/discounts" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const code = String(body.code || "").trim().toUpperCase();
    const type = body.type === "percent" ? "percent" : "fixed";
    const value = parseFloat(body.value);
    if (!code || !value) return adminJson({ error: "کد و مقدار تخفیف الزامیه." }, 400);
    const discount = { code, type, value, active: true, usageCount: 0, createdAt: Date.now() };
    await saveDiscount(discount, env);
    return adminJson({ ok: true, discount });
  }
  if (path === "/discounts/delete" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    await deleteDiscount(String(body.code || ""), env);
    return adminJson({ ok: true });
  }

  // ---------------- تیکت‌های پشتیبانی ----------------
  if (path === "/tickets" && request.method === "GET") {
    return adminJson({ tickets: await listAllTickets(env) });
  }
  if (path === "/tickets/reply" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const ticket = await addTicketMessage(String(body.id || ""), "admin", String(body.message || ""), env);
    if (!ticket) return adminJson({ error: "تیکت پیدا نشد." }, 404);
    return adminJson({ ok: true, ticket });
  }
  if (path === "/tickets/close" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    await closeTicket(String(body.id || ""), env);
    return adminJson({ ok: true });
  }

  // ---------------- تنظیمات (اجرت ساخت) ----------------
  if (path === "/settings" && request.method === "GET") {
    return adminJson({ settings: await getSettings(env) });
  }
  if (path === "/settings" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const settings = await getSettings(env);
    if (body.fee18 != null) settings.fee18 = parseFloat(body.fee18);
    if (body.fee24 != null) settings.fee24 = parseFloat(body.fee24);
    await saveSettings(settings, env);
    return adminJson({ ok: true, settings });
  }

  // ---------------- مدیریت ربات تلگرام (همکاران، بک‌آپ، پیام گروهی) ----------------
  if (path === "/telegram/admins" && request.method === "GET") {
    const coAdmins = await getCoAdmins(env);
    return adminJson({ owner: String(env.ADMIN_ID || ""), coAdmins });
  }
  if (path === "/telegram/admins" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const chatId = String(body.chatId || "").trim();
    const role = String(body.role || "");
    const validRoles = ["orders", "products", "full"];
    if (!/^\d+$/.test(chatId) || !validRoles.includes(role)) {
      return adminJson({ error: "chatId یا role نامعتبره (role باید orders/products/full باشه)." }, 400);
    }
    if (chatId === String(env.ADMIN_ID)) {
      return adminJson({ error: "این آیدی مالکه، نیازی به اضافه‌کردن نیست." }, 400);
    }
    const list = await getCoAdmins(env);
    ...
    await saveCoAdmins(list, env);
    try {
      await sendMessage(chatId, "سلام! دسترسی مدیریت به فروشگاه ریحون گلد گالری برات فعال شد.\nسطح دسترسی: " + ROLE_LABEL[role] + "\n\nبرای شروع /start رو بزن.", env);
    } catch (err) { /* اگه همکار هنوز به ربات پیام نداده، مهم نیست */ }
    return adminJson({ ok: true, coAdmins: list });
  }
  if (path === "/telegram/admins/delete" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const chatId = String(body.chatId || "");
    let list = await getCoAdmins(env);
    list = list.filter((a) => String(a.chatId) !== chatId);
    await saveCoAdmins(list, env);
    return adminJson({ ok: true, coAdmins: list });
  }
  if (path === "/telegram/backup" && request.method === "POST") {
    await runBackup(env, "داشبورد وب (دستی)");
    return adminJson({ ok: true });
  }
  if (path === "/telegram/broadcast" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const message = String(body.message || "").trim();
    if (!message) return adminJson({ error: "متن پیام خالیه." }, 400);
    await notifyAdmins(env, 1, "📢 پیام از داشبورد مدیریت:\n\n" + message);
    return adminJson({ ok: true });
  }

  return adminJson({ error: "مسیر پیدا نشد." }, 404);
}
